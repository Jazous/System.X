...
namespace System.X.Net
{
    public sealed class HttpHelper
    {
        ...
        public async Task<string> POST_FormData(string url, string token, IEnumerable<KeyValuePair<string, string>> formData, params FileInfo[] files)
        {
            ...
            if (formData != null && formData.Any())
                foreach (var data in formData)
                    content.Add(new StringContent(data.Value, Encoding.UTF8), data.Key);
